...
class RadarPreHead(nn.Module):
    def __init__(
        ...
        super(RadarPreHead, self).__init__()
    ...
    def forward_single(self, inputs, img_inputs, metas):
        ...
        lidar_feat = self.shared_conv(inputs)  # [1, 512, 180, 180]

        #################################
        # image to BEV # zy: no need to transfer again(did it before)
        #################################
        ...
        #################################
        # image guided query initialization # query initialization with bev_feat(lidar_feat)
        #################################
