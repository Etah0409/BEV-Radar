...
from .loading_utils import load_augmented_point_cloud, reduce_LiDAR_beams, get_nu_radar
import torch
...
class Loadnuradarpoints(object):
    ...
    def __init__(self,
                 ...
                 dataroot='/data/workspace/zhaoyuan/bev/mmdetection3d/data/nuscenes/',
    ...
    def __call__(self, results,):
        ...
        points, radar_tokens, times = self.get_nu_radar(results['sample_idx'], self.nusc, True, self.load_dim, filter=self.filter, timfuse=self.time_fusion)
        points = torch.cat([points, times], dim=0 )
        points[2 , :] = 0 #-0.15
        points = points[[0,1,2,5,15,8,9,18], :].transpose(0, 1).contiguous() # (x,y,z,rcs,cls,vxcpmp,vycpmp, t)
        #points = points[[0,1,2,5,15,8,9], :].transpose(0, 1).contiguous() # (x,y,z,rcs,cls,vxcpmp,vycpmp, )
        #points = points[[0,1,2,5,18], :].transpose(0, 1).contiguous() # (x,y,z,rcs,t )
        ...
        #####原始数据加入radar
        #results['points'].tensor = torch.cat([results['points'].tensor, results['radar_points'].tensor],dim=0)
        return results
...
class LoadMultiViewImageFromFiles:
    ...
    def __call__(self, results):
        ...
        """
        #if results['sample_idx'] == '30e55a3ec6184d8cb1944b39ba19d622':
        #    print("pause")
...
class LoadPointsFromMultiSweeps:
    ...
    def _load_points(self, lidar_path):
        ...
        """
        if '/data/workspace/zhaoyuan/bev/mmdetection3d' not in lidar_path:
            lidar_path = '/data/workspace/zhaoyuan/bev/mmdetection3d/' + lidar_path
